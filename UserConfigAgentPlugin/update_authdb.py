...
"""
Python module for installing/removing the UserConfigAgent mechanism
...
USERCONFIGAGENT_MECHANISM = 'UserConfigAgentPlugin:check,privileged'
...
def InstallPlugin(allow_admin_unlock, no_screensaver):
  ...
  for right in [AUTHENTICATE_RIGHT, LOGIN_DONE_RIGHT]:
    ...
    if not mechanisms.count(USERCONFIGAGENT_MECHANISM):
      mechanisms.append(USERCONFIGAGENT_MECHANISM)
...
def RemovePlugin(restore_screensaver):
  ...
  for right in [AUTHENTICATE_RIGHT, LOGIN_DONE_RIGHT]:
    data = _GetRightData(right)
    mechanisms = data.get('mechanisms', [])
    if mechanisms.count(USERCONFIGAGENT_MECHANISM):
      mechanisms.remove(USERCONFIGAGENT_MECHANISM)
      data['mechanisms'] = mechanisms
      if _SetRightData(right, data):
